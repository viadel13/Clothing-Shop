...
const ClothingList = () => {
  ...
  const dataLocal = JSON.parse(localStorage.getItem("ClothingData"));
    ...
    ? dataLocal.map((i) => {
        return (
          <div
            ...
            key={i.id}
          >
            <div className="article mt-5 placeholder-glow">
              <div className="image-holder d-flex justify-content-center"></div>
              <div className="details-articles">
                <h3 className="placeholder">
                  {i.title.split(" ")[0]} {i.title.split(" ")[1]}
                </h3>
                <h4 className="placeholder">${i.price}</h4>
                <button
                  className="ajouter-art placeholder"
